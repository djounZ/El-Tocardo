...
namespace AI.GithubCopilot.UnitTests.Infrastructure.Utils;
...
public class MockHttpMessageHandler : HttpMessageHandler
{
    ...
    public static MockHttpMessageHandler CreateWithStreamedJsonContent<T>(
        IEnumerable<T> items,
        HttpStatusCode statusCode = HttpStatusCode.OK,
        ...
        JsonSerializerOptions? options = null)
    {
        return new MockHttpMessageHandler((_, _) =>
        {
            // Simulate NDJSON (newline-delimited JSON)
            ...
            if (!string.IsNullOrWhiteSpace(endOfStream))
            {
                sb.AppendLine(endOfStream);
            }
