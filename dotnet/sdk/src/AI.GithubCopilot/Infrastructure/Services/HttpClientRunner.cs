...
public sealed class HttpClientRunner
{
    ...
    internal async Task<string?> SendAndReadAsStringAsync(
        HttpClient client,
        HttpMethod method,
    ...
    internal async  Task<string?> SendAndReadAsStringAsync<TIn>(
        TIn requestContent,
        HttpClient client,
        HttpMethod method,
    ...
    internal async  Task<TOut> SendAndDeserializeAsync<TOut>(
        HttpClient client,
        HttpMethod method,
        [StringSyntax(StringSyntaxAttribute.Uri)] string? requestUri,
        Dictionary<string, string> headers,
    ...
    internal async  Task<TOut> SendAndDeserializeAsync<TIn,TOut>(
        TIn requestContent,
        HttpClient client,
        HttpMethod method,
        [StringSyntax(StringSyntaxAttribute.Uri)] string? requestUri,
    ...
    internal async  IAsyncEnumerable<StreamItem<TOut>> SendAndReadStreamAsync<TIn,TOut>(
        TIn requestContent,
        HttpClient client,
        HttpMethod method,
        [StringSyntax(StringSyntaxAttribute.Uri)] string? requestUri,
        ...
        Func<StreamReader,CancellationToken, Task<StreamItem<TOut>>> contentReaderAsync, ILogger logger)
    {
        ...
        while (!reader.EndOfStream && !cancellationToken.IsCancellationRequested)
        {
            ...
            using var streamReader = new StreamReader(new MemoryStream(Encoding.UTF8.GetBytes(line)));
