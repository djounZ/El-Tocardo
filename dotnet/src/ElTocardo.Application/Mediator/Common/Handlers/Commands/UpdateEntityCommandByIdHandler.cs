...
public class UpdateEntityCommandByIdHandler<TEntity, TId, TKey, TCommand>(
    IEntityRepository<TEntity,TId, TKey> repository,
    ILogger<UpdateEntityCommandByIdHandler<TEntity, TId, TKey, TCommand>> logger,
    IValidator<TCommand> validator,
    AbstractDomainUpdateCommandMapper<TEntity,TId, TKey,TCommand> commandMapper)
    : CommandHandlerBase<TCommand>(logger) where TEntity: IEntity<TId,TKey> where TCommand : UpdateCommandBase<TId>
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var configuration = await repository.GetByIdAsync(command.Id, cancellationToken);
